...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (int i = 0; i < tokens.size(); i++){
			if(isWord(tokens.get(i))){
				this.numWords++;
				this.numSyllables = this.numSyllables+super.countSyllables(tokens.get(i));
			}
			else{
				this.numSentences++;
			}
	...
	public static void main(String[] args)
	{
		...
		testCase(new EfficientDocument("They're taking the hobbits to Isenguard, guard, guard"),13,9,1);
