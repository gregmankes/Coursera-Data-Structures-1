...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while(!queue.isEmpty() /*TODO: and we havent found word 2*/){
			current = queue.removeFirst();
			List<String> oneAway = this.nw.distanceOne(current.getWord(),true);
			for (String n: oneAway){
				if(!visited.contains(n)){
					child = current.addChild(n);
					visited.add(n);
					queue.add(child);
					if(n.equals(word2)){
